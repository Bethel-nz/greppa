...
### 3. Align Realtime retention

Set `lib/realtime.ts` `history.expireAfterSecs` to `resumeWindowMs / 1000` (down from the
mismatched 1 h). Realtime remains the live-tail transport; the ZSET is the durable replay.

### 4. Meta TTL to the window
...
(`GREPPA_RESUME_WINDOW_MS`, default 5 min); it is only referenced at `routes/chat.ts:60`.
...
1. Load meta. If missing or `conversationId` mismatch, emit `not_found` and return
   (unchanged gate). If meta is missing because the window elapsed, this is the
   `expired` path (see item 6).
...
3. Snapshot the ZSET: `ZRANGEBYSCORE msg:<id>:events (cursor +inf` — or the full log
   if the cursor is stale (see item 7). Emit each snapshot event as SSE.
...
When meta is gone (window elapsed), the stream returns a clean `expired` signal rather
than silence. The client then renders the finished message from the existing conversation
history load (`routes/chat/history.ts`, 2-day `sessionTtlMs`). This is the
"reconstruct for already finished" path: nothing to resume, nothing lost.
...
- **Stale-cursor guard:** if the client's cursor `seq` exceeds the log's current max seq
  (the log was reset by a retry), replay the full current log instead of ranging past it.
...
- `lib/config.ts` — rename `messageTtlMs` -> `resumeWindowMs` (`GREPPA_RESUME_WINDOW_MS`,
  default 5 min).
- `lib/emit.ts` — drop per-event ULID for `seq`; dual-write to `msg:<id>:events` ZSET with
  window TTL.
- `lib/realtime.ts` — `expireAfterSecs` = window.
- `routes/chat.ts` — meta TTL = window (via renamed config).
- `routes/chat/stream.ts` — seq cursor; subscribe-then-snapshot replay; `expired` signal.
- `routes/workflows/chat.ts` — terminal-status no-op guard; `DEL` events log at run start.
...
- Expired: reconnect after the window, assert the `expired` signal (not silence).
