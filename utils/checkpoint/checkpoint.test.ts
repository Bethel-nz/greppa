...
// a stable per-"user" key to mirror real usage (scopes/{id}/memory.mv2).
const K = (id: string) => `users/${id}/memory.mv2`
...
describe('Checkpoint', () => {
  ...
  test('write creates + uploads; a later read reuses the cache (no re-download)', async () => {
    ...
    const a = await cp.read(K('u1'), async (p) => dec.decode(await readFile(p)))
    const b = await cp.read(K('u1'), async (p) => dec.decode(await readFile(p)))
    expect(a).toBe('hello')
    expect(b).toBe('hello')
  ...
  test('read operates on an isolated snapshot; a concurrent write does not tear it', async () => {
    ...
    // Flip the canonical file while the read is mid-flight.
  ...
  test('etag conflict refreshes and retries once (last-writer-wins)', async () => {
    const storage = new MemoryStorage()
    const cp = new Checkpoint({ storage, cacheDir: await cacheDir(), maxOpen: 8, idleMs: 1000 })
    ...
    await cp.write(K('u1'), async (p) => writeFile(p, enc.encode('mine')))
    const got = await storage.get(key)
    expect(dec.decode(got!.body)).toBe('mine')
  })
